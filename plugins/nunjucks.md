...
```html
<div>{{ comp.button({ text: "Click here" }) | await | safe }}</div>
```
...
```html
{{ comp.button({ content: "Click here" }) | await | safe }}
```

The `await` filter is required because components are asynchronous and `safe`
