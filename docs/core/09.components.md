...
a button could be saved as `_components/button.vto`:
...
Vento automatically add the `await` keyword when printing a component.
...
To make this easier, Vento has the `comp` special tag that allows to consume
components similarly to JSX:
